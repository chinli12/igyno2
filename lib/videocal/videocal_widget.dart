...
import 'videocal_model.dart';
export 'videocal_model.dart';
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          FlutterFlowIconButton(
                            ...
                            onPressed: () async {
                              _model.chat = await ChatCall.call(
                                input: _model.textController.text,
                              );

                              if ((_model.chat?.succeeded ?? true)) {
                                _model.video = await VideCall.call(
                                  input: ChatCall.message(
                                    (_model.chat?.jsonBody ?? ''),
                                  ),
                                );

                                if ((_model.video?.succeeded ?? true)) {
                                  await Future.delayed(
                                      const Duration(milliseconds: 5000));
                                  _model.getvide = await GetvideoCall.call(
                                    id: VideCall.id(
                                      (_model.video?.jsonBody ?? ''),
                                    ),
                                  );

                                  FFAppState().videoURL = GetvideoCall.url(
                                    (_model.getvide?.jsonBody ?? ''),
                                  )!;
                                  safeSetState(() {});
